...
export class MemoryCard {
  ...
  _validateCard() {
    ...
    if (!this.superblock.isValidVersion) {
      this._setInvalidCardMessage(
        `The version ${this.superblock.version} is invalid and not in the format 1.x.0.0 on the memory card.`
      )
      return
    }
    ...
    if (!this.superblock.isValidCardSizeByTotalClusters) {
      this._setInvalidCardMessage(
        "The memory card must be larger than the total size of usable clusters."
      )
      return
    }
